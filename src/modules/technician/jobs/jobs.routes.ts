...
async function getOwnedJob(jobId: string, technicianId: string) {
...
techJobsRouter.patch(
  ...
  asyncWrapper(async (req, res) => {
    const profile = await getTechProfile(req.userId);
    const job = await getOwnedJob(req.params.jobId as string, profile.technician_id);
...
techJobsRouter.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const partIds = parts.map((p: { part_id: number }) => p.part_id);
    ...
    const created = await prisma.serviceRequestPart.createMany({
      data: parts.map((p: { part_id: number; quantity: number }) => ({
...
techJobsRouter.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const subtotal = invoiceItems.reduce(
      (sum: Decimal, item: { total_price: Decimal }) =>
