...
async function findVendorOrder(orderId: string, vendorId: string) {
...
vendorOrdersRouter.get(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const where: any = { warehouse_id: { in: warehouseIds } };
...
vendorOrdersRouter.get(
  ...
  asyncWrapper(async (req, res) => {
    const order = await prisma.order.findUnique({
      where: { order_id: req.params.orderId as string },
      include: {
        user: { select: { user_id: true, full_name: true, email: true } },
...
vendorOrdersRouter.post(
  ...
  asyncWrapper(async (req, res) => {
    ...
    const response: any = { message: "Order return processed, inventory restored" };
    if (result.length > 0) {
